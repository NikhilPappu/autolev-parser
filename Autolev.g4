...
varDecl:   varType varDecl2
(',' varDecl2)*;

varType: Newtonian|Frames|Bodies|Particles|Points|Constants|Specified
|Specifieds|Imaginary|Variables ('\'')*|MotionVariables ('\'')*;

varDecl2: ID (('{' INT ':' INT (',' INT ':' INT)* '}'))? 
('{' INT '}')? ('+'|'-')? ('\'')* ('=' expr)?;
...
massDecl2:  (ID '=' (INT|FLOAT)|ID '=' ID|ID '=' ID '=' (INT|FLOAT));

inertiaDecl: Inertia ID (',' expr)+;
...
Specified: ('S'|'s')('P'|'p')('E'|'e')('C'|'c')('I'|'i')('F'|'f')('I'|'i')('E'|'e')('D'|'d');
